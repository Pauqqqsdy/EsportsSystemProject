...
def update_round_robin_results(match):
    ...
    for m in completed_matches:
        ...
        # Проверяем, есть ли команды в статистике
        if m.team1.id not in team_stats or m.team2.id not in team_stats:
            print(f"  Пропускаем матч - команды не найдены в статистике")
            continue
...
def update_all_round_robin_results(table):
    """Обновляет все результаты в таблице Round Robin за один раз"""
    print(f"Обновляем все результаты для таблицы {table}")
    
    # Получаем все результаты команд в таблице
    all_results = RoundRobinResult.objects.filter(table=table)
    print(f"Найдено {all_results.count()} результатов команд в таблице")
